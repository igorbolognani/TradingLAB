...
import { AssetHelp, HelpDot, PaperCandleChart, type ChartCandle } from "./ui";
...
export default function PaperControl({ userMode = false }: { userMode?: boolean }) {
  ...
  return (
    <section className="page-stack paper-control-page">
      <div className="page-heading"><div><div className="eyebrow">Alpaca Paper / {userMode ? "connected workspace" : "private control room"}</div><h1>Paper workspace.</h1></div><span className={`api-badge ${status?.configured ? "api-online" : "api-offline"}`}><span />{status?.configured ? "Paper conectado" : "Paper aguardando configuração"}</span></div>
      ...
      <div className="paper-status-grid">
        <article className="panel"><div className="panel-kicker">Connection</div><h2>{status?.configured ? "Conexão protegida" : "Conexão não disponível"} <HelpDot label="Sobre a conexão"><span className="help-popover-copy">A conexão usa o backend do ambiente Paper. O navegador recebe apenas dados e status necessários para a tela.</span></HelpDot></h2><div className="paper-status-list"><span>Ambiente<strong>{status?.environment ?? "—"}</strong></span><span>Feed<strong>{status?.market_data_feed ?? "—"}</strong></span><span>Transporte<strong>{status?.transport ?? "—"}</strong></span><span>Realtime<strong>{status?.realtime_enabled ? "habilitado" : "desabilitado"}</strong></span></div>{status?.missing?.length ? <div className="execution-note"><strong>Configuração incompleta</strong><span>{status.missing.join(", ")}</span></div> : null}{userMode && !status?.configured ? <a className="button button-primary" href="/alpaca/connect">Conectar Alpaca Paper</a> : null}</article>
        <article className="panel"><div className="panel-kicker">Safety gate</div><h2>{orderLocked ? "Novas ordens bloqueadas" : "Paper pronto para envio"} <HelpDot label="Sobre o gate"><span className="help-popover-copy">O servidor valida kill switch, ambiente, allowlist, quantidade, valor e posição antes de aceitar uma ação.</span></HelpDot></h2><div className="safety-tags"><span>{status?.kill_switch ? "KILL SWITCH ON" : "KILL SWITCH OFF"}</span><span>{status?.live_enabled ? "LIVE INVALID" : "LIVE OFF"}</span><span>MAX ${status?.max_order_notional_usd ?? "—"}</span></div><div className="execution-note"><strong>{orderLocked ? `Bloqueado · ${lockReason}` : "Pronto para envio"}</strong><span>Cancelamento permanece disponível para reduzir risco.</span></div></article>
      </div>
      <div className="paper-grid">
        <article className="panel paper-market-panel"><div className="panel-heading"><div><div className="panel-kicker">Live quote / IEX</div><h2>Cotação monitorada</h2></div><button className="button button-outline" type="button" onClick={() => void refresh()} disabled={isRefreshing}>{isRefreshing ? "Atualizando…" : "Atualizar"}</button></div><div className="paper-controls"><label>Ativo <AssetHelp symbol={symbol} /><select value={symbol} onChange={(event) => setSymbol(event.target.value)}>{symbols.map((item) => <option key={item}>{item}</option>)}</select></label><label>Timeframe <HelpDot label="Sobre timeframe"><strong className="help-popover-title">Intervalo do candle</strong><span className="help-popover-copy">Cada vela resume negociações dentro de uma janela de tempo.</span><ul className="help-popover-list"><li><b>1Min:</b> mais detalhe e mais ruído.</li><li><b>5–15Min:</b> visão intraday mais legível.</li><li><b>1Hour:</b> contexto intraday amplo.</li><li><b>1Day:</b> uma vela por sessão.</li></ul><span className="help-popover-footnote">A última vela intraday pode ainda estar aberta.</span></HelpDot><select value={timeframe} onChange={(event) => setTimeframe(event.target.value)}><option>1Min</option><option>5Min</option><option>15Min</option><option>1Hour</option><option>1Day</option></select></label><button className="button button-primary" type="button" onClick={() => void loadBars()} disabled={!status?.configured}>Carregar candles</button></div>{quote ? <div className="quote-grid"><div><span>Último negócio</span><strong>{formatPrice(quote.quote.last_price)}</strong></div><div><span>Bid</span><strong>{formatPrice(quote.quote.bid_price)}</strong><small>{quote.quote.bid_size ?? "—"}</small></div><div><span>Ask</span><strong>{formatPrice(quote.quote.ask_price)}</strong><small>{quote.quote.ask_size ?? "—"}</small></div><div><span>Latência / idade</span><strong>{quote.latency_ms == null ? "—" : `${quote.latency_ms} ms`}</strong><small>{quote.data_age_seconds == null ? quote.transport : `${Math.round(quote.data_age_seconds)} s · ${quote.transport}`}</small></div></div> : <div className="paper-empty">Aguardando uma cotação real.</div>}{quote ? <div className="chart-footnote">{quote.message} Evento {formatTime(quote.quote.event_time)} · recebido {formatTime(quote.quote.receive_time_utc)}.</div> : null}</article>
      ...
      <article className="panel paper-order-panel"><div className="panel-kicker">Paper order gate</div><h2>Enviar uma ordem controlada <HelpDot label="Sobre ordens Paper"><strong className="help-popover-title">Fluxo protegido</strong><span className="help-popover-copy">A ação só passa quando o backend libera explicitamente as chaves de segurança. Quantidade, notional, posição e ambiente são verificados no servidor.</span></HelpDot></h2><div className="paper-order-form"><label>Ativo <AssetHelp symbol={symbol} /><select value={symbol} onChange={(event) => setSymbol(event.target.value)}>{symbols.map((item) => <option key={item}>{item}</option>)}</select></label><label>Lado <HelpDot label="Sobre o lado da ordem">Compra aumenta a posição; venda reduz uma posição existente. O backend valida posição e limites antes de qualquer envio.</HelpDot><select value={side} onChange={(event) => setSide(event.target.value as "buy" | "sell")}><option value="buy">Compra</option><option value="sell">Venda</option></select></label><label>Quantidade <HelpDot label="Sobre quantidade">Número inteiro de unidades. O limite de quantidade e o valor máximo são definidos pelo backend, não pelo navegador.</HelpDot><input type="number" min="1" step="1" value={quantity} onChange={(event) => setQuantity(event.target.value)} /></label><label>Tipo <HelpDot label="Sobre tipo de ordem">Market tenta executar ao preço disponível; Limit só aceita preço igual ou melhor que o limite informado.</HelpDot><select value={orderType} onChange={(event) => setOrderType(event.target.value as "market" | "limit")}><option value="market">Market</option><option value="limit">Limit</option></select></label>{orderType === "limit" ? <label>Preço limite <HelpDot label="Sobre preço limite">Preço máximo para compra ou mínimo para venda. A ordem pode permanecer aberta se o mercado não atingir esse preço.</HelpDot><input type="number" min="0.01" step="0.01" value={limitPrice} onChange={(event) => setLimitPrice(event.target.value)} /></label> : null}<button className="button button-primary" type="button" onClick={() => void submitOrder()} disabled={orderLocked || isSubmitting}>{isSubmitting ? "Enviando…" : orderLocked ? `Bloqueado · ${lockReason}` : "Enviar ordem Paper"}</button></div></article>
    </section>
  );
}
